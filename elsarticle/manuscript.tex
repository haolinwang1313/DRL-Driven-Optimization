...
\begin{abstract}
Surrogate-assisted optimization is increasingly used to explore block-scale urban energy design, yet benchmark conclusions remain fragile when optimizer ranking depends more on the chosen surrogate than on independent evaluation. This study examines that risk through a bounded comparison between Deep Deterministic Policy Gradient (DDPG) and fair-budget NSGA-II in a shared surrogate environment for urban morphology design, using Energy Use Intensity (EUIt), Energy Generation (EG), and Sunlight Hours (H) as the three target indicators. A deep neural network (DNN) surrogate maps 12 morphological factors to these indicators, and the final benchmark is conducted on the most accurate surrogate identified from a 500/1000/1500/2000-sample scale study. On the selected 2000-sample surrogate, NSGA-II achieves stronger objective summaries, the largest Hypervolume, and the lowest IGD, while all three DDPG scenarios remain weaker under post-hoc preference-aligned utility selection. Supplementary checkpoint-sensitivity analysis shows that surrogates with similar training-fit metrics can still differ in extrapolative bound-violation behavior, and a same-budget CMA-ES follow-up indicates that corner exploitation is not unique to DDPG. A small-batch physical probe adds bounded external support: EUIt aligns most consistently, the current EG check remains weaker but directionally stable, and a revised windowsill-style direct-sun-hours check places H in the same broad range as the surrogate values, albeit with weaker stability than EUIt. Taken together, the paper supports a methodological interpretation of surrogate-conditioned benchmark fragility in urban energy design, not a claim of general policy-learning superiority.
\end{abstract}
