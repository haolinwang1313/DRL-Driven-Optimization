...
The title retains `optimizer ranking` because ranking is standard terminology in optimization-benchmark contexts and the study examines relative method ordering under a shared surrogate condition. The main text reduces repeated `optimizer ranking` wording and instead uses `optimizer comparison`, `method comparison`, `relative performance`, and `method ordering` where the prose is about interpretation rather than title-level scope.
