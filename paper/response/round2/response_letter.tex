...
\end{tabular}
\end{center}
\tablecaptionstyle{\textbf{Revised Table 2 excerpt.} Evaluation modes are separated by source, output, use, and methodological limit.}
}
...
\revisionseparator
\revisionloc{Supplementary Information, Section S5.3 paragraph.}
...
\referenceitem{Wang, H., Wu, Z., Gu, W., et al. (2026). Towards climate-resilient cities: Assessing and planning urban energy demand under extreme climate events. \textit{Energy and Buildings}, 360, 117436. \responsedoi{https://doi.org/10.1016/j.enbuild.2026.117436}}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the descriptor-selection logic and dependency structure were not explained clearly enough. The original wording was problematic because the 12 morphology quantities are derived descriptors from generated block geometries rather than independent variables that can vary freely.

We revised Section 2.1, Table 1, and Supplementary Sections S1.2--S1.5 to rewrite the descriptor explanation, define the descriptors with units and construction notes, and add supporting material on prototype settings, OSLI, dependency, PCA coverage, and distributions.
...
\CompactDescriptorTable
\revisionseparator
\revisionloc{Supplementary Information, Table S2 excerpt.}
\begin{center}
\footnotesize
\begin{tabular}{lll}
\toprule
...
\end{center}
\revisionseparator
\revisionloc{Supplementary Information, Fig.~S1.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the functions of Table 1 and Table 2 were not sufficiently distinct. The original wording was problematic because readers need to know which table defines morphology inputs and which table defines evaluation modes.

We revised Tables 1 and 2 and Sections 2.1--2.2 to separate descriptor construction from evaluation-mode interpretation.
...
\revisionseparator
\revisionloc{Supplementary Information, Table S4 excerpt.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the cross-validation explanation should be understandable beyond the metric values. The original wording was problematic because small error numbers are not enough unless the reader knows what was held out and what source layer was being tested.

We revised Section 2.3, Table 3, Fig. 5, and Supplementary Sections S2.2--S2.5 to explain repeated five-fold cross-validation as repeated held-out testing of the selected 2000-sample DNN surrogate against analytic target values.
...
This keeps the validation claim tied to the analytic target layer and prevents the cross-validation results from being read as physical validation evidence.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\revisionseparator
\revisionloc{Supplementary Information, Section S2.2 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the framework components needed to be introduced before the detailed results. The original wording was problematic because the workflow is easier to audit when each result is tied to the representation and evaluation mode that produced it.

We revised the Methodology opening, Fig. 1, Table 2, and Section 2.6 to rewrite the workflow as a sequence of feasible morphology generation, descriptor extraction, analytic response assignment, surrogate training, optimizer querying, projection, and selected sensitivity analyses.

The revised workflow lets the reader trace whether a result comes from the analytic benchmark, the selected DNN surrogate, feasible projection, physical stress testing, or climate sensitivity. The full 2000-sample benchmark is now explicitly described as analytic response assignment plus surrogate prediction, not a set of direct annual physical simulations. This component order is now consistent across the workflow figure, evaluation-mode table, and methodology text.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for pointing this out. We agree that the previous manuscript used reinforcement-learning terminology without a sufficient reference basis for Markov Decision Processes. We therefore added foundational references for Markov Decision Processes and reinforcement learning in the Literature review. We also revised the surrounding text so that the citation supports the general RL background without implying that the present morphology task is a natural physical-time MDP.

The revised manuscript cites Puterman's \textit{Markov Decision Processes: Discrete Stochastic Dynamic Programming} and Sutton and Barto's \textit{Reinforcement Learning: An Introduction}. These sources support the general sequential-decision and reinforcement-learning context. The manuscript then immediately states the modelling boundary: our DDPG implementation serializes static surrogate queries rather than representing physical-time morphology control.

We also revised Section 2.4 to define the actual surrogate-query environment. The state is the normalized three-target vector, the action is an absolute 12-dimensional descriptor query, the next state is returned by the guarded surrogate, and one episode contains 40 surrogate interactions. This makes the modelling boundary explicit: the study uses DDPG in a static surrogate-query setting, not as a dynamic urban morphology evolution model.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we added foundational MDP and reinforcement-learning references in the Literature review. Second, we revised the DDPG framing to state that the morphology task is not treated as physical-time control. Third, we expanded Section 2.4 so that state, action, reward, episode, and target-network updates are defined within the static surrogate-query environment.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Section 1.2 paragraph.}
Deep reinforcement learning provides another family of search methods, but its role in morphology design requires careful framing. The revised manuscript now cites foundational Markov Decision Process and reinforcement-learning references at this point, including Puterman and Sutton and Barto. The paragraph then clarifies that the morphology task in this study is not treated as a physical-time design-control process. DDPG is used as a serialized static black-box search method on a guarded surrogate response surface.
\revisionseparator
...
\responseheading{Local references.}
\begin{responsereferences}
\referenceitem{Puterman, M. L. (1994). \textit{Markov Decision Processes: Discrete Stochastic Dynamic Programming}. Wiley. \responsedoi{https://doi.org/10.1002/9780470316887}}
\referenceitem{Sutton, R. S., and Barto, A. G. (2018). \textit{Reinforcement Learning: An Introduction}. 2nd ed. MIT Press. ISBN 9780262039246.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the figure and table units needed clearer labelling. The original wording was problematic because unit ambiguity makes it hard to distinguish raw targets, normalized coordinates, and morphology descriptors.

We revised Figs. 4, 5, and 9, Tables 1, 3, and 6, and Supplementary captions and tables to standardize target units, morphology units, one-to-one reference lines, and figure captions.
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the DDPG episode definition could be mistaken for physical time evolution. The original wording was problematic because in this static surrogate benchmark an episode is a query sequence rather than an annual or dynamic building-operation process.

We revised Sections 2.4 and 3.2, Figs. 2 and 6, and Supplementary Section S3.1 to define one episode as 40 surrogate-query steps and align the training-curve captions with that definition.

The revised explanation makes the reward at each step depend on the surrogate-predicted normalized target vector and preference weighting. Because each episode contains 40 surrogate-query steps, the plotted training quantity is cumulative episode return rather than a single-step reward. The episode axis therefore describes training progress in descriptor-query space, not a physical calendar or dynamic urban system.
...
Figure 6 describes how the DDPG agents interact with the guarded surrogate during training. Across the three preference scenarios, episode returns increase rapidly during early training and then enter a high-return region with visible seed-to-seed variability. Because each episode contains a fixed sequence of 40 surrogate queries, the trajectories represent policy-training behaviour on a static response surface. They should therefore be read as search trajectories over descriptor queries, not as the physical evolution of one block design.
...
\figcaptionstyle{\textbf{Revised Fig.~6.} DDPG training trajectories across the three preference scenarios. Each episode contains 40 surrogate-query steps, and the plotted return is the sum of the step rewards in that episode.}
\revisionseparator
\revisionloc{Supplementary Information, Section S3.1 paragraph.}
...
\revisionseparator
\revisionloc{Supplementary Information, Table S5 excerpt.}
\ClimateTable
\revisionseparator
\revisionloc{Supplementary Information, Section S6.3 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this suggestion. We agree that the previous manuscript was written too narrowly for readers who are interested in urban energy design but may not follow every detail of DDPG, HV, IGD, or surrogate-response construction. We therefore rewrote the paper so that the urban planning and energy-design question appears before the optimizer details.

A central motivation of this revision is to explore how AI-assisted search can support early-stage urban planning. At the planning stage, decision makers often need to screen morphology options before detailed building-system design, detailed HVAC assumptions, or full district-energy models are available. The revised manuscript therefore presents DDPG and NSGA-II not only as algorithms, but also as tools for understanding how AI-based and Pareto-based search behave under the same surrogate-conditioned design workflow.

This broader framing is intended for several reader groups. AI and optimization researchers can see how policy learning behaves in a static surrogate-query environment. Urban planners and urban designers can see why continuous descriptor-space candidates must be mapped back to generated feasible blocks. Building-energy researchers can see how analytic target assignment, surrogate prediction, and selected physics-based stress testing are separated. Policy-oriented readers can see why climate sensitivity, rooftop PV potential, and sunlight access matter before detailed design information is available.

We also revised the presentation to improve accessibility. The front matter now contains a Nomenclature block, Table 1 defines morphology descriptors compactly, Table 2 separates evaluation modes, and the Discussion explains what can and cannot be inferred from descriptor-space, projection, physical, and climate analyses. These changes make the paper more readable as an urban energy design study, while keeping the interpretation cautious.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made five corresponding revisions. First, we reorganized the Introduction around Motivation, Literature review, and Contributions and paper organization. Second, we reframed the study as an early-stage design-support workflow that can be read by AI researchers, urban planners, building-energy researchers, and policy-oriented readers. Third, we replaced the front-matter abbreviation list with Nomenclature. Fourth, we revised the method tables and figure captions to reduce terminology ambiguity. Fifth, we rewrote the Discussion and Conclusions to separate descriptor-space findings, feasible-projection interpretation, physical stress-test results, and climate sensitivity.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\reviewerpage{2}
\checkresponse{%
We thank Reviewer 2 for the careful and constructive assessment of the core methodology. These comments identified the main weaknesses of the previous manuscript: the data-generation workflow was not stated clearly enough, the physical-check terminology was too strong, the sample-generation process was underdescribed, the RL formulation could be misread as a natural sequential-control problem, and the objective definitions were not sufficiently explicit. We have revised the manuscript substantially in response. The revised version now separates analytic target assignment, surrogate prediction, feasible projection, selected physics-based calculations, and cross-climate sensitivity, and it reduces all conclusions to the level supported by these evaluation modes.
}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this important comment. We agree that the previous manuscript did not explain the analytic approximation with enough precision. In the original version, the relationship among generated morphologies, analytic target assignment, the DNN surrogate, and selected physical calculations was not sufficiently transparent. This could make readers infer that all 2000 benchmark samples were produced by direct annual EnergyPlus and Radiance simulations, which is not the intended interpretation.

We have therefore rewritten the data-generation description. The revised manuscript states that feasible block morphologies are generated first and converted into 12 morphology descriptors. EUIt, EG, and H are then assigned by an analytic response generator. This analytic response surface provides a controlled target-assignment layer for comparing optimizers under a shared surrogate environment. The reasons for using this layer are reproducibility, computational stability, and the need to support repeated optimizer queries under the same benchmark conditions. DDPG and NSGA-II each require many surrogate calls across seeds or runs, and direct annual physical simulation for all generated morphologies and all optimizer queries would not be a practical comparison protocol.

We also added the interpretation boundary explicitly. The analytic response-generation dataset does not replace direct annual EnergyPlus and Radiance simulation. Instead, it defines the benchmark target regime used to train the selected DNN surrogate and compare the optimizers. We then added selected physics-based calculations later in the workflow to examine how analytic target values relate to EnergyPlus- and Radiance-component outputs for locked generated blocks.

The revised Supplementary Information now gives the analytic response equations, stochastic terms, and clipping ranges. Together with the revised workflow figure and evaluation-mode table, this makes the data source and its limitation explicit. The dataset is therefore not direct annual EnergyPlus and Radiance simulation for the full 2000-sample benchmark.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we rewrote Section 2.1 to clarify that block layouts are generated before descriptors are calculated. Second, we rewrote Section 2.2 to define analytic response generation as target assignment, not direct physical simulation. Third, we revised Table 2 so that analytic response generation, surrogate prediction, feasible projection, physical stress testing, and climate sensitivity are separated. Fourth, we added the response equations, noise terms, and clipping ranges in Supplementary Section S1.4.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Fig.~1.}
...
\figcaptionstyle{\textbf{Revised Fig.~1.} Workflow linking feasible morphology generation, analytic response assignment, surrogate-assisted optimization, feasible-block projection, and sensitivity analysis.}
\revisionseparator
\revisionloc{Main manuscript, Section 2.1 paragraph.}
The morphology generator first constructs feasible block layouts and only then calculates descriptor values. Each block is represented as a 240 m $\times$ 240 m square divided into a $3\times3$ grid of 80 m $\times$ 80 m land units. One cell is assigned as open space, and the remaining eight cells are populated with residential prototypes.
\revisionseparator
...
\EvaluationModeTable
\revisionseparator
\revisionloc{Supplementary Information, Section S1.4 opening paragraph.}
...
\revisionseparator
\revisionloc{Supplementary Information, analytic equations and clipping ranges.}
\AnalyticEquations
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for raising this point. We agree that the terms physical probe and physical-stack probe were too strong and could suggest a more complete physical evaluation than the study actually performed. We also recognize that the earlier workflow figures and descriptions did not separate the analytic benchmark layer from the selected physics-based calculations clearly enough. We have corrected this throughout the revised manuscript.

The revised manuscript now uses the term limited physics-based cross-model stress test. This wording is deliberate. The calculation compares analytic response-generator values with selected EnergyPlus- and Radiance-component outputs for locked generated blocks. It does not represent a full physical simulation of every benchmark sample and it does not certify optimizer rankings under a complete physical design workflow.

We also revised the relevant figures, tables, and section structure. Fig. 1 now shows analytic response and surrogate modelling as a separate stage from feasible design assessment. Table 2 defines the evaluation modes and separates analytic response generation, surrogate prediction, feasible projection, physical stress testing, and climate sensitivity. Section 2.6 explains that the physical stress test uses 24 locked cases, including 18 direct feasible cases and 6 optimizer-linked cases. Supplementary Section S5 then documents the completed outputs and the EG limitation.

A key correction is the EG target. The locked physical workflow did not produce annual roof-plane irradiance for the direct stress-test cases. We therefore do not call the physical EG result a full roof-irradiance calculation. It is reported as a GHI-based rooftop-PV proxy, and this limitation is stated directly in the Supplementary Information.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we replaced validation-style physical-probe wording with limited physics-based cross-model stress test. Second, we revised Fig. 1 and Table 2 to separate analytic response generation from selected physics-based calculations. Third, we revised Section 2.6 and Section 3.5 to state the 24-case scope and the direct/optimizer-linked split. Fourth, we added the EG proxy limitation in Supplementary Section S5.4 and removed wording that implied broad physical validation.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Fig.~1.}
\compactfig{0.82}{fig1.pdf}
\figcaptionstyle{\textbf{Revised Fig.~1.} The workflow separates analytic target assignment from later selected physics-based calculations.}
\revisionseparator
\revisionloc{Main manuscript, Table 2 excerpt.}
\EvaluationModeTable
\revisionseparator
...
\revisionseparator
\revisionloc{Main manuscript, Fig.~9.}
\compactfig{0.82}{physical_cross_model_stress_test.pdf}
\figcaptionstyle{\textbf{Revised Fig.~9.} Cross-model stress test for 18 direct generated feasible blocks.}
\revisionseparator
\revisionloc{Supplementary Information, Section S5.4 paragraphs.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this important criticism. We agree that the previous manuscript did not provide sufficient validation evidence for the strength of the language used in the original text. The selected physical calculations show noticeable deviations from the analytic target values, and the revised manuscript now reports those deviations directly instead of using them as confirmation.

We revised Sections 2.6, 3.5, and 4.2, Fig. 9, Table 6, and Supplementary Section S5 to report the completed physical cases, the direct feasible-case metrics, and the downgraded interpretation. The direct feasible subset contains 18 locked generated blocks. For these cases, EUIt has MAE 3.788, nMAE 0.340, and Spearman $\rho=-0.404$; the EG GHI proxy has MAE 0.441, nMAE 0.405, and Spearman $\rho=-0.023$; and H has MAE 1.108, nMAE 0.601, and Spearman $\rho=-0.043$.

These values support a stress-test conclusion about fragility, not physical equivalence or optimizer-ranking transfer. The revised Results section therefore states that agreement is limited and target-dependent. The Discussion now treats the physical calculations as a boundary check on the analytic benchmark rather than as a validation of surrogate-conditioned optimizer rankings.

We also separated the 18 direct feasible cases from the 6 optimizer-linked cases. The direct cases compare analytic target values with physics-based outputs without projection. The optimizer-linked cases decompose the route from descriptor-space candidate to nearest generated block and then to the physics-based result. Keeping these two subsets separate prevents the response from overstating what the small-batch calculation can support.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\figcaptionstyle{\textbf{Revised Fig.~9.} Cross-model stress test for 18 direct generated feasible blocks.}
\revisionseparator
\revisionloc{Main manuscript and Supplementary Information, physical summary excerpt.}
\PhysicalSummaryTable
\revisionseparator
\revisionloc{Supplementary Information, Section S5.2 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the original sample-generation description was too brief. The previous manuscript could be read as if the 12-dimensional descriptor vectors were sampled directly from a generic numerical design space. That is not the workflow used here, and we have revised the text to make the generation order clear.

The revised manuscript now explains that the primary object is a generated feasible block. Each block is a 240 m $\times$ 240 m square divided into a 3 by 3 grid of 80 m $\times$ 80 m land units. One cell is assigned as open space, and the remaining cells receive residential prototypes, floors, and orientation values under the generation rules. The 12 morphology descriptors are then calculated from the generated geometry. The samples are therefore random generated feasible morphologies represented by derived descriptors, not Latin-hypercube, grid-based, or unconstrained descriptor samples.

We also added coverage and dependency checks. The 2000-row canonical pool has no exact duplicate descriptor rows. OSLI is integer-valued by construction. Six principal components are required to explain 95 percent of descriptor variance. The descriptor-dependency table reports algebraic residuals near numerical roundoff for FAR, BD, AF, OSR and related geometry quantities. Fig. S1 summarizes descriptor distributions, OSLI counts, and normalized nearest-neighbour distances.

This revision also explains why feasible projection is needed later. The optimizers operate on continuous descriptor candidates, while the design object is a generated feasible block. Therefore, continuous optimizer outputs are mapped back to nearest generated feasible blocks before morphology-level interpretation.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made five corresponding revisions. First, we expanded the morphology-generation protocol in Section 2.1. Second, we simplified Table 1 to define descriptor, unit, calculation, and note. Third, we added descriptor coverage and PCA results in Section 3.1 and Fig. 4. Fourth, we added prototype settings, descriptor dependencies, and descriptor distributions in Supplementary Sections S1.1 to S1.5. Fifth, we linked the sampling description to the feasible-projection analysis in Section 3.4.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Section 2.1 paragraphs.}
...
\figcaptionstyle{\textbf{Revised Fig.~4.} Descriptor coverage and cross-validated surrogate predictions.}
\revisionseparator
\revisionloc{Supplementary Information, Section S1.2 paragraph.}
...
\revisionseparator
\revisionloc{Supplementary Information, Table S2 and Fig.~S1 excerpts.}
\begin{center}
\footnotesize
\begin{tabular}{lll}
\toprule
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this important methodological comment. We agree that the original reinforcement-learning description could suggest a naturally sequential urban morphology design process, which is not the correct interpretation of our implementation. The problem studied here is essentially static surrogate-based morphology search, and we have revised the manuscript to make that explicit.

In the revised manuscript, DDPG is described as serialized static black-box search on a guarded surrogate response surface. The state is the normalized three-target vector returned by the surrogate. The action is an absolute 12-dimensional normalized descriptor query, not an incremental modification of the previous morphology. The next state is obtained by denormalizing the descriptor action, evaluating it with the guarded surrogate, and normalizing the predicted EUIt, EG, and H values. One episode contains 40 surrogate-query steps. The episode therefore defines the training horizon for actor-critic updates, not the physical evolution of an urban block.

We also revised the reward definition. The manuscript now defines normalized target coordinate \(z_i\), ideal vector \(\mathbf{u}=[0,1,1]^\mathsf{T}\), weighted normalized distance \(d_w\), and reward \(R=1-d_w\). The previous reward-offset interpretation has been removed. The reward is also distinguished from the fixed-domain post-hoc utility used later for benchmark comparison.

This revision keeps DDPG in the study but narrows its interpretation. We use DDPG as a continuous-action policy-learning comparator under a matched query budget, not as a physical-time control model of morphology evolution.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we rewrote Section 2.4 around the static surrogate-query environment. Second, we revised Figs. 2 and 3 to explain the serialized query workflow and actor-critic update. Third, we corrected the reward equations. Fourth, we revised Section 3.2 and Supplementary Section S3.1 to clarify that episodes describe training behaviour on a static response surface.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Section 2.4 paragraph.}
...
\revisionseparator
\revisionloc{Main manuscript, Section 3.2 DDPG training paragraph.}
Because each episode contains a fixed sequence of 40 surrogate queries, the trajectories represent policy-training behaviour on a static response surface. They should therefore be read as search trajectories over descriptor queries, not as the physical evolution of one block design.
\revisionseparator
\revisionloc{Supplementary Information, Section S3.1 paragraph.}
Each DDPG scenario uses 20 seeds, 600 episodes, and 40 surrogate interactions per episode. The seed-level summaries record best episode return, final episode return, plateau episode, and best-to-final regression. These quantities describe policy-training behaviour under serialized surrogate queries on a static surrogate response surface.
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the terminology in the previous manuscript was confusing. Several earlier terms could be read as stronger than the calculations actually supported. We have removed or replaced this language and reorganized the method description around explicitly separated evaluation modes.

The revised manuscript now uses a consistent vocabulary. Analytic response generation refers to the target-assignment layer used to generate EUIt, EG, and H values for the 2000-sample benchmark. Surrogate prediction refers to the selected DNN surrogate used during optimizer queries. Feasible projection refers to the nearest-feasible-block mapping from descriptor-space candidates to generated blocks. The physics-based cross-model stress test refers only to the 24 locked cases. Cross-climate sensitivity refers only to the four selected blocks evaluated under Harbin, Beijing, and Guangzhou weather.

We added Table 2 to make these distinctions visible. The table is not an assumption table. It is an evaluation-mode table that states source, output, use, and limitation. This table, together with the Nomenclature, is intended to prevent the revised manuscript from mixing analytic target assignment, surrogate prediction, projection, physical calculation, and climate sensitivity.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we added and revised Nomenclature for the central terms and symbols. Second, we rewrote Table 2 as an evaluation-mode table. Third, we aligned Sections 2.2, 2.6, 3.5, and 4.2 with the same terminology. Fourth, we removed wording that implied broad physical confirmation or broad physical optimizer ranking.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Nomenclature excerpt.}
...
\begin{tabularx}{0.96\linewidth}{@{}p{0.14\linewidth}X p{0.13\linewidth}X@{}}
\toprule
\multicolumn{4}{@{}l}{\textit{Selected abbreviations and terms}}\\
EUIt & Energy Use Intensity & EG & Energy Generation \\
DDPG & Deep Deterministic Policy Gradient & NSGA-II & Non-dominated Sorting Genetic Algorithm II \\
HV & Hypervolume & IGD & Inverted Generational Distance \\
GHI & global horizontal irradiance & OSLI & open-space location index \\
\bottomrule
\end{tabularx}
\end{center}
\revisionseparator
...
\revisionseparator
\revisionloc{Main manuscript, Section 2.2 paragraph.}
This module provides a controlled morphology-to-performance response surface for optimizer comparison. The nested 500/1000/1500/2000-sample pools are deterministic subsets of the same generated morphology pool under the recorded random seed and morphology-generation protocol.
\revisionseparator
\revisionloc{Main manuscript, Section 2.6 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the objective definitions were not sufficiently transparent. The previous manuscript stated that EUIt, EG, and H conceptually correspond to energy demand, rooftop generation, and sunlight-hour quantities, but it did not show the analytic formulas used to assign the target values in the generated dataset. This has now been corrected.

The revised manuscript states that the 2000-sample benchmark uses analytic response generation. The full equations are provided in Supplementary Section S1.4. The equations define orientation penalty, aspect-ratio average, morphology-dependent openness term, density penalty, and the three unclipped responses \(EUIt^\ast\), \(EG^\ast\), and \(H^\ast\). The final values are then clipped to the benchmark training-domain ranges: EUIt from 66.0 to 96.0, EG from 1.2 to 2.85, and H from 6.0 to 7.85.

We also added the stochastic-term convention and objective direction. Lower EUIt is preferred, while higher EG and H are preferred. These definitions are now connected to the DNN surrogate and optimizer objective space. The formulas therefore make the benchmark reproducible, while the manuscript remains clear that these are analytic target-assignment equations, not direct annual EnergyPlus and Radiance outputs.

This revision also makes the objective directions explicit before the optimizer comparison. The surrogate and optimizers use the analytic target values for a controlled benchmark, while the later physics-based calculations test selected locked cases under a different evaluation layer.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we revised Section 2.2 to identify analytic response generation as target assignment. Second, we added objective directions and target definitions. Third, we added the full EUIt, EG, and H equations in Supplementary Section S1.4. Fourth, we added the clipping equations and stochastic-term convention used by the generated dataset.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Section 2.2 paragraph.}
The response directions follow the design objectives: lower EUIt is preferred, while higher EG and H are preferred. The active 2000-sample pool is the largest nested sample regime used for the selected DNN surrogate. The full equations, station-bias convention, stochastic terms, and clipping ranges are given in Supplementary Section S1. Table 2 distinguishes analytic response assignment, surrogate prediction, feasible projection, and the post-optimization sensitivity analyses.
\revisionseparator
\revisionloc{Supplementary Information, Section S1.4 equations.}
\AnalyticEquations
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this suggestion. We agree that the previous manuscript did not provide a sufficiently concrete reproducibility statement. In the revision, we rewrote the Data and code availability section to state what will be released through the project GitHub repository and what will be excluded for security or redistribution reasons. The revised statement also names the repository location so that the release route is clear before the public package is finalized.

The public release will include the code required to regenerate the analytic response dataset, surrogate assessment summaries, optimizer comparison tables, feasible-projection results, physics-based stress-test summaries, cross-climate sensitivity summaries, and figure-data tables. It will also include processed datasets, result tables, source hashes, configuration files required for reproducible local runs, and documentation explaining the workflow.

We also state the exclusion boundary. Credentials, private execution logs, private configuration files, machine-specific directories, and non-redistributable raw inputs will not be released. This boundary is necessary because the project uses physical calculation workflows and environment files that must not be exposed. The revised statement therefore commits to a usable public reproducibility package while avoiding unsafe release of credentials or private machine details.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we rewrote the Data and code availability statement around the public GitHub repository. Second, we specified the categories of code, processed data, result tables, figure data, and scripts that will be released. Third, we clarified which private or restricted materials are excluded from public release.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Data and code availability.}
The code, processed datasets, result tables, and figure-data files supporting the numerical analyses will be made publicly available through the project repository at \href{https://github.com/haolinwang1313/DRL-Driven-Optimization}{https://github.com/haolinwang1313/DRL-Driven-Optimization}. The public release will include scripts and configuration files for regenerating the analytic response dataset, surrogate assessment summaries, optimizer comparison tables, feasible-projection results, physics-based stress-test summaries, and cross-climate sensitivity summaries. Local credentials, server-specific logs, machine-specific paths, and non-redistributable raw inputs will be excluded from the public package.
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the figure spelling and label errors needed correction. The original wording was problematic because workflow and architecture figures define the method structure, so visible errors reduce clarity.

We revised Fig. 1, Fig. 3, and their captions to correct the typo and align figure labels with the revised methodology and DDPG terminology.
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the earlier utility interpretation could be read as a direct optimizer-ranking result. The original wording was problematic because benchmark fairness requires separating scalarized utility, equal-size HV/IGD, retained outputs, and feasible projection.

We revised Sections 3.3 and 3.4, Table 5, Figs. 7 and 8, and Supplementary Sections S3 and S5 to rewrite the comparison around fixed-reference descriptor-space metrics, equal-size diagnostics, and projection.
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the reward formula needed correction. The original wording was problematic because the previous expression could be misread as an artificial large-offset reward.

We revised Section 2.4 and Fig. 3 to define the reward directly from normalized weighted distance.

The revised equations normalize targets to $z_i$, use the ideal vector $[0,1,1]^\mathsf{T}$, compute $d_w$ with the preference vector, and set $R=1-d_w$ with no large offset. This aligns the manuscript with the static surrogate-query implementation and makes the reward scale interpretable across all three preference settings.

The response also explains why the correction matters: a bounded distance-based reward is interpretable across preference settings, while the removed offset could obscure the actual optimization signal. The verified training curves use the same reward contract as the revised equation. Because each episode contains 40 surrogate-query steps, the plotted training quantity is the cumulative episode return rather than a single-step reward.

The same notation is now used in the method text, equation block, and Fig. 3 explanation.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we corrected the reward equations in Section 2.4. Second, we revised Fig. 3 so that the reward path follows normalization, ideal-vector comparison, weighted distance, and $R=1-d_w$. Third, we verified the Fig. 6 and Supplementary Fig. S4 training logs against the same reward contract. Fourth, we removed the confusing large-offset notation from the response and manuscript text.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that target and morphology units were not consistently presented. The original wording was problematic because unit inconsistency makes it harder to audit results and compare figures.

We revised Tables 1, 3, 5, and 6, Figs. 4--10, and Supplementary tables and captions to standardize target units, morphology units, figure axes, and table headings.
...
\revisionseparator
\revisionloc{Supplementary Information, Table S5 note.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this important comment. We agree that the previous manuscript did not use the terms simulation, analytic approximation, and physical probing consistently enough. This was a substantive problem, not only a wording issue. In the original version, several figure labels and method descriptions could make readers believe that the full 2000-sample benchmark was generated through direct EnergyPlus and Radiance simulation. That was not the correct interpretation of the dataset used for the optimizer comparison.

We have revised the manuscript to separate three layers. The first layer is feasible morphology generation, where residential block layouts are generated and converted into 12 morphology descriptors. The second layer is analytic response generation, where EUIt, EG, and H are assigned to the generated morphology pool through explicit response equations and then used for surrogate training and optimizer comparison. The third layer is selected physics-based and climate-sensitive calculation, where locked generated blocks are examined through the Grasshopper/Rhino-Honeybee/EnergyPlus/Radiance workflow.

This revision affects the Abstract, Fig. 1, Table 2, Section 2.2, Section 2.6, Section 3.5, Discussion, and Supplementary Sections S1 and S5. The wording now consistently states that the benchmark dataset is analytic response-generation based, while the physical workflow is used for the 24-case cross-model stress test and the three-climate sensitivity analysis. We also removed terms that could imply full physical-stack validation or physical ranking transfer.

The revised interpretation is deliberately conservative. The analytic response surface supports a controlled optimizer comparison. The physics-based calculations then show how selected generated blocks behave under EnergyPlus and Radiance components. Because the physical stress test shows weak metric agreement and unsupported ranking transfer, the manuscript no longer presents descriptor-space optimizer ranking as direct physical performance ranking.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made five corresponding revisions. First, we revised the Abstract so that the dataset is described as morphology-to-performance response assignment rather than direct physical simulation. Second, we revised Fig. 1 so that analytic response and surrogate modelling are separated from feasible design assessment. Third, we added Table 2 to distinguish analytic response generation, surrogate prediction, feasible projection, physical stress testing, and climate sensitivity. Fourth, we revised Section 2.6 and Section 3.5 to describe the physical workflow as a limited cross-model stress test. Fifth, we added Supplementary Section S5.4 to state that the EG physical output is a GHI-based rooftop-PV proxy because annual roof-plane irradiance was unavailable in the locked physical workflow.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\revisionloc{Main manuscript, Fig.~1.}
\compactfig{0.86}{fig1.pdf}
\figcaptionstyle{\textbf{Revised Fig.~1.} Workflow separating feasible morphology generation, analytic response assignment, surrogate-assisted optimization, feasible-block projection, and sensitivity analysis.}
\revisionseparator
\revisionloc{Main manuscript, Table 2 excerpt.}
\EvaluationModeTable
\revisionseparator
\revisionloc{Main manuscript, Section 2.2 analytic response paragraph.}
The response directions follow the design objectives: lower EUIt is preferred, while higher EG and H are preferred. The active 2000-sample pool is the largest nested sample regime used for the selected DNN surrogate. The full equations, station-bias convention, stochastic terms, and clipping ranges are given in Supplementary Section S1. Table 2 distinguishes analytic response assignment, surrogate prediction, feasible projection, and the post-optimization sensitivity analyses.
\revisionseparator
\revisionloc{Main manuscript, Section 2.6 physical stress-test paragraph.}
The physical workflow is used as a selected cross-model stress test rather than as the generator of the optimizer benchmark. Locked generated blocks are reconstructed in the Grasshopper/Rhino-Honeybee workflow and evaluated through EnergyPlus and Radiance components where available. The results are therefore interpreted as selected physical probes of representative generated morphologies, not as full physical simulation for all analytic benchmark samples.
\revisionseparator
\revisionloc{Main manuscript, Discussion boundary paragraph.}
These two sensitivity analyses do not convert the descriptor-space comparison into a physical ranking result. They instead define where the analytic response surface, generated feasible blocks, and selected physical workflows agree only partially, which is the boundary needed for early-stage design-support interpretation.
\revisionseparator
\revisionloc{Supplementary Information, Section S5.4 paragraph.}
EG is summarized with the GHI-based rooftop-PV proxy in the stress-test table. This proxy preserves a generation-related comparison across the locked cases, but it should be interpreted as a simplified rooftop-PV indicator rather than a full annual roof-irradiance result.
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the previous Introduction did not make the necessity of the study sufficiently clear. In the original manuscript, the motivation was too close to an optimizer comparison narrative. It did not explain clearly enough why block-scale urban energy planning needs a representation-aware comparison between policy learning and Pareto search.

We have therefore rewritten the Introduction rather than adding only a short motivation sentence. The revised Introduction is organized into Motivation, Literature review, and Contributions and paper organization. The Motivation subsection starts from the urban energy problem: block morphology affects building energy demand, rooftop photovoltaic potential, winter sunlight access, envelope exposure, roof availability, and obstruction. These factors matter for early-stage planning because urban renewal, PV deployment, and climate-resilient planning decisions often have to be made before detailed building-system design is available.

The revised Literature review then explains why existing block-scale energy planning studies do not fully answer the present question. Simulation-based optimization, surrogate-assisted design, Pareto search, and DRL control have each been studied extensively. However, it remains less clear how optimizer ranking should be interpreted when policy learning and Pareto search operate on the same surrogate response surface, when the inputs are morphology descriptors derived from generated blocks, and when continuous descriptor outputs must be projected back to feasible morphologies.

The revised contribution is therefore not another claim that one optimizer is universally better. The study addresses a narrower and necessary problem: how to interpret optimizer ranking in early-stage urban energy design when surrogate prediction, descriptor representation, feasible-block projection, physical stress testing, and climate sensitivity all affect the meaning of the result.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we rewrote the Introduction into Motivation, Literature review, and Contributions and paper organization. Second, we expanded the urban energy planning motivation around building demand, rooftop PV potential, winter sunlight access, Chinese urban renewal, and climate-resilient planning. Third, we repositioned the literature review to compare simulation-based optimization, surrogate-assisted optimization, Pareto search, and DRL control. Fourth, we revised the contribution statement so that the study is framed as representation-aware early-stage design support rather than a broad optimizer-superiority claim.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\revisionseparator
\revisionloc{Main manuscript, Contributions paragraph.}
This study develops a surrogate-assisted comparison workflow for block-scale urban energy design. The contributions are threefold: a unified workflow linking feasible morphology generation, analytic target assignment, surrogate-assisted optimization, and post-optimization assessment; a controlled comparison between DDPG and NSGA-II under a shared surrogate; and a representation-aware interpretation that separates descriptor-space rankings, feasible-block projection, and selected physical and climate-sensitive calculations.
\revisionseparator
\revisionloc{Main manuscript, Discussion paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this important comment. We agree that the original manuscript did not explain how the selected baseline climate could affect the results or how the findings should be interpreted for other climate zones. In response, we added a limited cross-climate physical sensitivity analysis and revised the manuscript to state its scope clearly.

The added analysis keeps the block morphology fixed and changes only the weather file. The baseline physical workflow uses Dongtai station weather to represent the case-study weather condition. We then evaluated the same four selected generated block morphologies under Harbin, Beijing, and Guangzhou weather files. These cities provide three contrasting Chinese climate contexts: severe-cold, cold, and hot-summer warm-winter conditions. The surrogate was not retrained, and the morphology set was not reselected after observing the climate results.

The added calculations produced 12 additional case-weather combinations. The results show that demand is strongly climate sensitive. Relative to the Dongtai baseline, Harbin increases mean EUIt by 55.681 kWh m\(^{-2}\) y\(^{-1}\), Beijing increases it by 13.467 kWh m\(^{-2}\) y\(^{-1}\), and Guangzhou decreases it by 2.972 kWh m\(^{-2}\) y\(^{-1}\). EG ordering remains stable across the three additional climates, with Spearman \(\rho=1.0\) in each case. H ordering is less stable, with Spearman \(\rho=0.2\), \(0.4\), and \(0.2\) for Beijing, Guangzhou, and Harbin. EUIt ordering also changes under Harbin, where Spearman \(\rho=0.2\).

These results support a limited climate-sensitivity interpretation. They show how selected block morphologies respond when the physical weather input changes, but they do not establish cross-climate surrogate generalization. We state this boundary explicitly in the revised Discussion.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we added the cross-climate protocol in Section 2.6. Second, we added the cross-climate results in Section 3.5 and Fig. 10. Third, we added weather metadata, station-level changes, case-level results, and rank-stability analysis in Supplementary Section S6. Fourth, we revised the Discussion to state that the current results support selected-block climate sensitivity rather than general cross-climate transfer.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, Section 2.6 paragraph.}
The baseline weather file used in the physical workflow serves as the reference condition for the cross-climate sensitivity analysis. The cross-climate sensitivity set then evaluates the same four block morphologies under three additional weather files representing Harbin, Beijing, and Guangzhou. The surrogate is not retrained, and the morphology set is not reselected after observing climate results.
\revisionseparator
\revisionloc{Main manuscript, Section 3.5 climate paragraph.}
Relative to the Dongtai baseline, Harbin increases mean EUIt by 55.681 kWh m\(^{-2}\) y\(^{-1}\), Beijing increases mean EUIt by 13.467 kWh m\(^{-2}\) y\(^{-1}\), and Guangzhou decreases mean EUIt by 2.972 kWh m\(^{-2}\) y\(^{-1}\). EG ordering remains stable across the additional climates, while EUIt and H ordering are more climate dependent. The result is interpreted as selected-block climate sensitivity rather than evidence of cross-climate surrogate generalization.
\revisionseparator
\revisionloc{Main manuscript, Fig.~10.}
\compactfig{0.86}{cross_climate_sensitivity.pdf}
\figcaptionstyle{\textbf{Revised Fig.~10.} Cross-climate sensitivity of four selected blocks relative to baseline weather.}
\revisionseparator
\revisionloc{Supplementary Information, Section S6.1 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for raising this broader energy-planning issue. We agree that distributed energy systems, EV charging, storage, and district-level energy networks are increasingly important in China and can strongly affect urban energy planning. The current study approaches the problem from the building-demand and morphology side, so these energy-system interactions were not included as optimization objectives in the present model.

We revised the manuscript to make this scope explicit. The current objective system focuses on three early-stage morphology-related quantities: building energy demand represented by EUIt, rooftop generation potential represented by EG, and winter solar access represented by H. These objectives are appropriate for studying how block morphology affects building-side energy performance before detailed energy-system design is available. They do not include storage dispatch, EV charging schedules, district power exchange, electricity tariffs, or coupled energy-economic operation.

We also expanded the future-work discussion. A natural next step is to connect the current morphology-demand workflow with distributed energy systems. In that extension, building demand-side outputs from the present framework could become inputs to a district energy model that includes PV generation, storage operation, EV charging demand, grid exchange, and economic objectives. This would allow future studies to evaluate not only morphology-induced demand and rooftop generation potential, but also energy cost, peak load, flexibility, and carbon impacts under distributed-energy operation.

We did not add these systems as if they were already modelled, because that would overstate the present work. Instead, the revised Discussion explains that the current study provides the morphology and demand-side screening layer, while energy network coupling and EV-related economic analysis remain important extensions.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we clarified that the present objective system is limited to EUIt, EG, and H, which are building-demand, rooftop-generation, and sunlight-access quantities. Second, we revised the Discussion to state that distributed energy, EV charging, storage, grid exchange, and energy-economic operation are outside the current calculation scope. Third, we added these elements as future extensions that can build on the demand-side morphology workflow developed in this study.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\revisionseparator
\revisionloc{Main manuscript, Data and code availability scope statement.}
The public package will include the code, processed datasets, result tables, figure-data files, scripts, and configuration notes needed to reproduce the analytic response dataset, surrogate assessment summaries, optimizer comparisons, feasible-projection results, physical stress-test summaries, and cross-climate sensitivity summaries. Private credentials, restricted raw inputs, and machine-specific execution details are excluded.
\end{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the phrase best scenario was ambiguous in the previous manuscript. In an optimizer comparison with multiple objectives and multiple output representations, best cannot be defined by a single universal criterion. A candidate can be best under a scalarized preference reward, while another method can be stronger under descriptor-space HV, equal-size IGD, feasible-block projection, or selected physical sensitivity.

We therefore removed the ambiguous best-scenario wording and replaced it with explicit criteria. In the revised manuscript, DDPG outputs are interpreted as best scalarized candidates retained from individual seeds. NSGA-II outputs are interpreted as final populations and archive-based descriptor-space results. Table 4 defines the retained-output contract for each method, and Table 5 separates descriptor-space HV and IGD from fixed-domain utility. Fig. 8 then shows how descriptor-space candidates compress after nearest-feasible-block projection.

The revised interpretation is more precise. NSGA-II is strong in descriptor-space HV and IGD under the fixed benchmark reference. DDPG remains preference dependent. Projection changes the morphology-level interpretation because a large descriptor archive can collapse to a smaller feasible-block set. Therefore, the paper no longer asks which scenario is universally best. It asks which criterion is being used, what representation the criterion belongs to, and whether projection or physical sensitivity changes the interpretation.

This also helps planning-oriented readers. Instead of presenting a single best scenario, the revised workflow distinguishes between candidate screening, Pareto archive coverage, feasible-block diversity, and selected physics-based behaviour. These are different decision layers in early-stage urban energy planning.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we removed ambiguous best-scenario wording. Second, we revised Section 2.5 and Table 4 to define retained-output contracts. Third, we revised Section 3.3, Table 5, and Fig. 7 to separate fixed-domain utility, HV, and IGD. Fourth, we revised Section 3.4 and Fig. 8 to show that feasible-block projection changes morphology-level interpretation.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
...
\BenchmarkTable
\revisionseparator
\revisionloc{Main manuscript, Section 3.3 paragraph.}
Table 5 separates scalarized fixed-domain utility from descriptor-space HV and IGD under the shared benchmark reference. This distinction is important because a best scalarized DDPG candidate and a broad NSGA-II archive answer different comparison questions. The revised text therefore reports each criterion under its own retained-output contract rather than reducing the optimizer comparison to a single best-scenario label.
\revisionseparator
\revisionloc{Main manuscript, Section 3.4 paragraph.}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the previous manuscript did not compare the proposed workflow with existing approaches in enough detail. In the revision, we expanded the Literature review and Discussion to make the comparison clearer and more cautious.

Existing urban energy planning approaches provide several important foundations. Simulation-based optimization can evaluate urban form in relation to energy demand, daylight, and PV potential. Surrogate-assisted optimization can reduce the computational cost of repeated performance evaluation. NSGA-II and other evolutionary multi-objective methods can provide explicit Pareto archives. DRL methods have been widely used in building and energy-system control problems. These approaches are valuable, but they often address different representation and decision layers.

The present study contributes from the building demand-side and morphology-screening perspective. It does not replace high-fidelity district energy-system planning. Instead, it asks how early-stage morphology candidates should be screened when AI-based and Pareto-based optimizers operate on the same surrogate response surface. The revised framework separates descriptor-space search, feasible-block projection, selected physics-based stress testing, and climate sensitivity. This separation helps avoid reading a surrogate archive as direct feasible morphology diversity or direct physical performance ranking.

The advantage of the workflow is therefore not comprehensive superiority over previous planning frameworks. Its value is transparency: it shows how optimizer ranking depends on the response surface, retained-output contract, projection to feasible blocks, and selected physical or climate-sensitive calculations. This building demand-side perspective can later be coupled with distributed energy, EV charging, storage, grid exchange, and energy-economic optimization to support broader urban energy planning.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we expanded the Literature review to compare simulation-based optimization, surrogate-assisted design, Pareto search, and DRL control studies. Second, we revised the Discussion to explain the building demand-side perspective of the present workflow. Third, we clarified that the framework supports early-stage morphology screening and does not replace full district energy-system planning. Fourth, we added future-work language on coupling morphology-driven demand outputs with distributed energy, EV charging, storage, grid exchange, and economic analysis.
}
\responseheading{Relevant revised manuscript and supporting material.}
\begin{revisionbox}
\revisionloc{Main manuscript, literature review paragraph on simulation-based optimization.}
...
\revisionseparator
\revisionloc{Main manuscript, literature review paragraph on surrogate-assisted optimization.}
Surrogate modelling has expanded this field by reducing evaluation cost and enabling larger design sweeps, while coordinated surrogate studies demonstrate its value for coupled building and microclimate problems. Recent district-scale morphology optimization work further shows that surrogate-assisted frameworks can reduce the computational cost of exploring morphology effects on building energy demand. These studies provide the computational basis for the present work, but they leave open how method ordering should be interpreted when the comparison itself is conditioned by a surrogate response surface.
\revisionseparator
...
\referenceitem{Wang, H., Wu, Z., Gu, W., Liu, P., Sun, Q., \& Wang, W. (2026). A surrogate-assisted framework for district-scale urban morphology optimization toward reduced building energy demand. \textit{Applied Energy}, 422, 128294. \responsedoi{https://doi.org/10.1016/j.apenergy.2026.128294}}
...
\responsepara{%
We again thank the editor and reviewers for their careful evaluation and constructive suggestions. The revised manuscript, figures, tables, equations, references, Supplementary Information, and response letter have been updated to make the data-generation process, optimizer formulation, evaluation modes, physical and climate sensitivity analyses, and remaining limitations explicit.

We are especially grateful for the comments that identified ambiguity in the analytic response-generation dataset, physical-stress-test terminology, RL formulation, climate limitation, and broader planning relevance. These comments led to substantial revisions of the manuscript structure and interpretation. We hope that the revised package now provides a clearer and more cautious basis for evaluating surrogate-conditioned optimizer comparisons in block-scale urban energy design.
}
