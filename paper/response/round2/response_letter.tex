...
\responsepara{%
Table R1 is only an editor-facing overview. The evidence for each reviewer comment appears under the corresponding comment below, where revised manuscript excerpts, SI excerpts, figures, compact tables, equations, and local references are shown next to the response that uses them.
}
...
\checkresponse{%
We thank the reviewer for this comment. We agree that the original Abstract was too long and did not state the significance with enough precision. This point matters because the Abstract is the first place where readers decide whether the paper is a broad optimizer-superiority claim or a bounded surrogate-conditioned benchmarking study.

We revised the Abstract, final Introduction contribution paragraph, and Conclusions to reorder the abstract around the block-scale design problem, generated descriptors, analytic response surface, selected 2000-sample DNN surrogate, optimizer comparison, feasible projection, 24-case cross-model stress test, and four-block climate sensitivity analysis. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now reports repeated-validation nMAE values of 0.0176, 0.0265, and 0.0145 for EUIt, EG, and H; NSGA-II near-ceiling descriptor-space HV under the fixed reference; compression of the 2000-candidate NSGA-II archive to 51 unique feasible blocks after projection; weak physical metric and rank agreement; and climate-dependent EUIt and H responses. The revised wording therefore keeps the significance visible while avoiding a claim of broad physical optimizer superiority.
}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the previous front-matter list addressed abbreviations but did not sufficiently support the mathematical notation used in the methodology and optimization equations. This point matters because a reader needs both acronym definitions and symbol definitions before reading the analytic response and DDPG reward equations.

We revised the front matter and Table 1 to replace the Abbreviations block with a Nomenclature block containing Abbreviations and Formula symbols subsections. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now defines EUIt, EG, H, DNN, DDPG, NSGA-II, HV, IGD, nMAE, OSLI, the morphology descriptor vector, analytic target vector, normalized target coordinate, target bounds, ideal vector, axis-scaling vector, weighted distance, DDPG reward, orientation angle, response-generation noise term, and geometric quantities used by the descriptors. We kept full descriptor construction in Table 1 so that the Nomenclature supports mathematical readability without duplicating the descriptor table.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we replaced the front-matter Abbreviations block with a Nomenclature block. Second, we added a Formula symbols subsection for the analytic-response and DDPG reward notation. Third, we retained the detailed construction of the 12 morphology descriptors in Table 1 and added a cross-reference from Nomenclature to Table 1.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
\revisionloc{Main manuscript, Nomenclature excerpt.}
\textbf{Nomenclature} now contains two subsections. \textit{Abbreviations} defines EUIt, EG, H, DNN, DDPG, NSGA-II, CMA-ES, GHI, HV, IGD, nMAE, and OSLI. \textit{Formula symbols} defines the morphology descriptor vector $\mathbf{x}$, analytic target vector $\mathbf{y}^{ana}$, target value $y_i$, normalized target coordinate $z_i$, target bounds $y_i^{\min}$ and $y_i^{\max}$, ideal vector $\mathbf{u}$, axis-scaling vector $\mathbf{w}$, weighted distance $d_w$, DDPG reward $R$, orientation angle $\theta$, response-generation noise term $\epsilon$, and the geometric area, height, and canyon-width quantities used by the descriptor definitions.
\revisionseparator
\revisionloc{Main manuscript, Nomenclature-to-Table 1 cross-reference.}
The morphology descriptor symbols FAR, SD, AF, AR$_{ew}$, AR$_{ns}$, SVF, BD, OSR, SC, PAR, $\theta$, and OSLI are defined in Table~1.
\revisionseparator
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the original Fig. 1 caption was too brief for a workflow figure that carries much of the methodological structure. This point matters because the figure introduces the distinction between generated block geometries, morphology descriptors, surrogate-based optimizer queries, feasible-block projection, and selected sensitivity analyses.

We revised the methodology transition before Fig. 1 and the Fig. 1 caption to rewrite the transition paragraph and caption so that the workflow stages are introduced in order. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now identifies feasible morphology generation, analytic response assignment and DNN surrogate training, surrogate-assisted optimization, nearest-feasible-block projection, and limited physical and climate sensitivity analyses. The caption no longer implies that all 2000 generated samples were direct annual EnergyPlus and Radiance simulations.

This placement was deliberate. The workflow figure is now treated as the reader's map for the whole revision, so the response explains not only that the caption was lengthened, but also which methodological misunderstanding the new caption prevents.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we inserted a clearer methodology transition paragraph before Fig. 1. Second, we rewrote the Fig. 1 caption so that each workflow module is explicitly identified. Third, we made the distinction between analytic response generation and selected physical calculations visible before the detailed methodology.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the manuscript needed a clearer urban-energy and software context. This point matters because the work should read as a block-scale planning study motivated by urban renewal, PV deployment, and climate-sensitive design decisions rather than a generic optimizer exercise.

We revised Sections 1.1, 1.2, 2.2, and 2.6 to expand the urban morphology, building energy, rooftop PV, winter solar access, surrogate modelling, and simulation-based optimization context. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now clarifies that EnergyPlus- and Radiance-component calculations are used only for selected locked cases in the cross-model stress test and climate sensitivity analysis. The analytic response surface remains a controlled benchmark layer and is not presented as a substitute for detailed final-design simulation.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we expanded the Introduction around Chinese urban renewal, PV-oriented planning, and climate resilience. Second, we revised the methodology wording so that analytic response generation and selected physical calculations are separated. Third, we added literature support for surrogate-assisted optimization and urban-form energy analysis without presenting the workflow as a full physical design engine.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
We revised Section 2.1, Table 1, and Supplementary Sections S1.2--S1.5 to rewrite the descriptor explanation, define the descriptors with units and construction notes, and add supporting material on prototype settings, OSLI, dependency, PCA coverage, and distributions. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows that FAR, BD, AF, and OSR share footprint and gross-floor-area quantities, that OSLI is integer-valued by construction, that six principal components explain 95 percent of descriptor variance, and that exact duplicate descriptor rows are absent in the canonical 2000-row pool. Optimizer outputs are therefore interpreted first in descriptor space and then through nearest-feasible-block projection before morphology-level conclusions are drawn.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
We revised Tables 1 and 2 and Sections 2.1--2.2 to separate descriptor construction from evaluation-mode interpretation. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now Table 1 now defines morphology descriptors by unit or type, construction, and interpretation, while Table 2 distinguishes analytic response generation, surrogate prediction, feasible projection, physical stress testing, and climate sensitivity by source, output, and boundary. This split avoids mixing descriptor construction, target assignment, surrogate prediction, and selected physical calculations in one undifferentiated method description.

This table separation also makes later evidence easier to follow because response-surface construction, surrogate prediction, and physical stress testing now have different table anchors.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the methodological assumptions and validation hierarchy were not visible enough. This point matters because surrogate accuracy on analytic targets cannot be treated as physical equivalence or optimizer-ranking transfer.

We revised Sections 2.2, 2.3, 2.6, 3.5, and 4.2, Tables 3 and 6, Fig. 5, Fig. 9, and Supplementary Sections S1, S2, and S5 to separate analytic response generation, DNN surrogate validation against analytic targets, feasible projection, and limited physics-based cross-model stress testing. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now reports repeated 5 by 5 validation nMAE values of 0.0176, 0.0265, and 0.0145 for EUIt, EG, and H, then reports weak physical agreement in the 18 direct feasible cases with Spearman rho values of -0.404 for EUIt, -0.023 for the EG proxy, and -0.043 for H. The physical calculations are therefore interpreted as a stress test showing fragility, not as validation of direct physical optimizer superiority.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the cross-validation explanation should be understandable beyond the metric values. This point matters because small error numbers are not enough unless the reader knows what was held out and what evidence source was being tested.

We revised Section 2.3, Table 3, Fig. 5, and Supplementary Sections S2.2--S2.5 to explain repeated five-fold cross-validation as repeated held-out testing of the selected 2000-sample DNN surrogate against analytic target values. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now keeps the repeated nMAE and Spearman metrics and adds OSLI holdout, outer-shell holdout, and feature-tail holdout checks for more difficult descriptor regions. The evidence assesses DNN agreement with analytic targets, not agreement with EnergyPlus or Radiance outputs.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the reinforcement-learning formulation and references needed more conservative framing. This point matters because the static design-search problem should not be overstated as a natural time-evolving MDP.

We revised Sections 1.2 and 2.4 and Fig. 3 to add RL/DDPG references and rewrote DDPG as a policy-learning comparator in a bounded surrogate-query environment. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now defines state as a serialized descriptor vector, action as a descriptor-space candidate proposal, reward as normalized weighted target distance, and an episode as a fixed sequence of surrogate queries. DDPG is therefore used as a continuous-action query generator under the fixed surrogate, not as a model of physical urban evolution.

This conservative wording keeps the RL reference useful while making clear that the present benchmark is a static design-query setting, not a physical control process.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the DDPG symbols and equations were too implicit. This point matters because readers need the actor-critic components and reward path to understand how DDPG queries the surrogate benchmark.

We revised Section 2.4 and Fig. 3 to define the state, action, reward, actor, critic, replay buffer, and target networks and redraw Fig. 3 around this architecture. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows target normalization, the ideal vector $[0,1,1]^\mathsf{T}$, the weighted normalized distance $d_w$, and the corrected reward $R=1-d_w$. The formulation explains query scoring in the selected surrogate environment without implying that DDPG learns physical building behaviour.

The response therefore links the corrected equations to the architecture figure, so readers can trace how a descriptor query becomes a surrogate prediction and then a scalar reward.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
We revised Figs. 4, 5, and 9, Tables 1, 3, and 6, and Supplementary captions and tables to standardize target units, morphology units, one-to-one reference lines, and figure captions. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now labels EUIt in kWh m$^{-2}$ y$^{-1}$, EG in $10^6$ kWh y$^{-1}$ where that scale is used, H in hours, and morphology quantities by their unit or type. This correction changes presentation only; the underlying numerical results are unchanged.
...
We revised Sections 2.4 and 3.2, Figs. 2 and 6, and Supplementary Section S3.1 to define one episode as 40 surrogate-query steps and align the training-curve captions with that definition. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now makes the reward at each step depend on the surrogate-predicted normalized target vector and preference weighting. The episode axis therefore describes training progress in descriptor-query space, not a physical calendar or dynamic urban system.
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the generality of the selected cases and climate analysis needed clearer limits. This point matters because the benchmark dataset and the smaller physical or climate-sensitive case set answer different questions.

We revised Sections 2.6, 3.5, 4.2, and 4.3, Fig. 10, Table 6, and Supplementary Section S6 to add a four-block by three-climate sensitivity analysis and state that building types and climate-aware surrogate retraining remain outside the current scope. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows relatively stable EG ordering in the four-block set but climate-dependent EUIt and H responses, especially under severe-cold conditions. The analysis is a bounded sensitivity check for selected locked cases, not nationwide climate-zone generalization.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we defined the four-block by three-climate sensitivity design. Second, we added Fig. 10, Table 6, and Supplementary Section S6 material. Third, we revised the Discussion to identify building-type expansion and climate-aware surrogate training as future work.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the manuscript needed to be easier for Applied Energy readers to follow. This point matters because the original version introduced acronyms, workflow stages, optimizer terminology, and validation language too densely.

We revised the Abstract, Nomenclature, Introduction, Table 2, Discussion, and Supplementary Information structure to rewrite the reader entry points, add Nomenclature, separate descriptor definitions from evaluation modes, and integrate limitations into the Discussion. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now presents the problem, response surface, surrogate, optimizer comparison, projection, stress-test, and climate-sensitivity boundary in a more linear sequence. The paper remains a surrogate-assisted benchmarking and design-support study rather than a broad DRL-superiority claim.

These readability changes also support the reviewer response itself: the same terminology now appears in the front matter, tables, method sections, and evidence excerpts, reducing the chance that readers encounter different labels for the same evidence layer.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made four corresponding revisions. First, we rewrote the Abstract and Introduction for a clearer reader entry point. Second, we replaced the front-matter abbreviation list with Nomenclature. Third, we separated descriptor and evaluation-mode explanations into Tables 1 and 2. Fourth, we revised the Discussion and Conclusions so that limitations are integrated with the main interpretation.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
We revised Sections 2.1--2.3, Table 2, and Supplementary Section S1.4 to define generated feasible morphologies, analytic target assignment, selected DNN surrogate training, and the boundary between analytic and physical evidence. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now documents the target functions, stochastic terms, clipping ranges, descriptor dependencies, nested sample regime, and common surrogate checkpoint used for DDPG and NSGA-II. The analytic response dataset is not presented as direct physical simulation of every candidate.
...
We revised Table 2, Sections 2.6 and 3.5, Table 6, Fig. 9, and Supplementary Sections S5.3--S5.4 to replace physical-probe wording with limited physics-based cross-model stress test and rewrote the interpretation around selected locked cases. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now compares EnergyPlus- and Radiance-component outputs against analytic target values for those cases. The stress test identifies weak agreement and ranking fragility; it does not support direct physical transfer of optimizer rankings.
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that the previous validation evidence was insufficient and too optimistic. This point matters because the physical calculations should be reported as weak agreement where the tested cases show that result.

We revised Sections 2.6, 3.5, and 4.2, Fig. 9, Table 6, and Supplementary Section S5 to report completed physical cases, direct feasible-case metrics, and the downgraded interpretation. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows 18 direct feasible cases with EUIt MAE 3.788, nMAE 0.340, and Spearman rho -0.404; EG proxy MAE 0.441, nMAE 0.405, and rho -0.023; and H MAE 1.108, nMAE 0.601, and rho -0.043. These values support a stress-test conclusion about fragility, not physical equivalence or optimizer-ranking transfer.
...
We revised Sections 1.2, 2.4, and 3.2, Figs. 2, 3, and 6, and Supplementary Section S3.1 to define state, action, episode, reward, actor, critic, replay buffer, and target networks for the static surrogate benchmark. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now states that one episode is a sequence of 40 surrogate queries and that the reward is computed from the surrogate-predicted normalized target vector. DDPG is included as a policy-learning comparator under a matched query budget, not as a model of dynamic building operation.
...
We revised Table 2 and Sections 2.2, 2.6, 3.5, and 4.2 to standardize the evaluation terms and assign each one a source, output, and interpretation boundary. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now uses analytic response generation for EUIt/EG/H target assignment, surrogate prediction for the DNN layer, feasible projection for descriptor-to-block mapping, physical stress test for locked cases, and climate sensitivity for weather-file changes. This terminology prevents overclaiming by keeping evidence layers separate.

This terminology cleanup also supports the reviewer-response evidence blocks because each displayed excerpt now has a named evidence source: analytic formula, surrogate result, projection diagnostic, physical stress test, or climate sensitivity.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
We revised Section 2.2, Table 2, and Supplementary Section S1.4 to add the analytic response-generation equations, stochastic terms, objective directions, and clipping ranges. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now defines the orientation penalty, aspect-ratio average, openness and solar-access proxy terms, density penalty, response-generation noise, and final clipped outputs. The formulas define a controlled benchmark layer and are not claimed to be a full physical energy model.

The response also states the remaining boundary: exposing the formulas improves reproducibility of the analytic benchmark, but those equations are still target-assignment rules rather than a calibrated physical simulation model. This is why the response keeps both the equations and the clipping ranges in the evidence package: the formulas define the target surface, while the ranges define the benchmark values seen by the surrogate and optimizers.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
We revised the Data and code availability statement, README, and response manifest to rewrite the availability wording around a sanitized public GitHub release. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now states that scripts and processed tables for the analytic response dataset, surrogate assessment, optimizer comparison, feasible projection, physical stress-test summaries, and cross-climate sensitivity summaries will be made available. Raw credentials, private logs, machine-specific paths, and non-redistributable inputs remain excluded.
...
We revised Fig. 1, Fig. 3, and their captions to correct the typo and align figure labels with the revised methodology and DDPG terminology. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now Fig. 1 now uses consistent workflow-module labels, while Fig. 3 uses standardized labels for actor, critic, replay buffer, target networks, state, action, and reward path. This is a presentation correction and does not change the method or results.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
We revised Sections 3.3 and 3.4, Table 5, Figs. 7 and 8, and Supplementary Sections S3 and S5 to rewrite the comparison around fixed-reference descriptor-space metrics, equal-size diagnostics, and projection. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows NSGA-II strong in descriptor-space HV and IGD under the selected surrogate benchmark while DDPG remains preference dependent and projection changes feasible-block interpretation. Projection and physical stress-test evidence prevent descriptor-space ranking from being read as direct physical performance ranking.
...
We revised Section 2.4 and Fig. 3 to define the reward directly from normalized weighted distance. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now normalizes targets to $z_i$, uses the ideal vector $[0,1,1]^\mathsf{T}$, computes $d_w$ with the preference vector, and sets $R=1-d_w$ with no large offset. This aligns the manuscript with the static surrogate-query implementation and makes the reward scale interpretable.
...
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we corrected the reward equations in Section 2.4. Second, we revised Fig. 3 so that the reward path follows normalization, ideal-vector comparison, weighted distance, and $R=1-d_w$. Third, we removed the confusing large-offset notation from the response and manuscript evidence.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
We revised Tables 1, 3, 5, and 6, Figs. 4--10, and Supplementary tables and captions to standardize target units, morphology units, figure axes, and table headings. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now reports EUIt as kWh m$^{-2}$ y$^{-1}$, EG using the stated $10^6$ kWh y$^{-1}$ scale where applicable, H in hours, and morphology quantities by unit or type. The numerical results are unchanged; the revision improves auditability.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
...
\responseheading{Response.}
\checkresponse{%
We thank the reviewer for this comment. We agree that analytic, simulation, and physical wording needed consistent use. This point matters because the evidence source changes the interpretation of every optimizer and validation statement.

We revised the Abstract, Sections 1--4, Table 2, Table 6, and Supplementary Sections S1 and S5 to standardize analytic response generation, surrogate prediction, physical stress test, and climate sensitivity terminology. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now makes clear that analytic response generation assigns targets to the generated pool, the surrogate predicts those targets during optimization, and selected locked blocks are examined through physical calculations and weather-file changes. This distinction supports the revised boundary: surrogate rankings are meaningful within the selected benchmark, while physical stress-test results show weak agreement and unsupported ranking transfer.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we standardized analytic response generation, surrogate prediction, physical stress test, and climate sensitivity terminology. Second, we revised Table 2 and Table 6 to carry the same evidence-source distinction. Third, we removed wording that could imply that the 2000 analytic samples were direct physical simulations.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
We revised Sections 1.1--1.3 and the Discussion to rewrite the motivation, contribution, and positioning around representation-aware design support. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows why optimizer rankings depend on descriptor coverage, retained-output contracts, feasible-block projection, surrogate checkpoint, and evaluation mode. The contribution is a design-support and benchmarking contribution, not a universal superiority claim for DDPG or NSGA-II.
...
We revised Sections 2.6, 3.5, and 4.2, Fig. 10, Table 6, and Supplementary Section S6 to add a four-block by three-climate sensitivity calculation and report weather metadata. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows stable EG ordering in the tested four-block set but climate-dependent EUIt and H responses, with the severe-cold case producing the largest EUIt increase. The surrogate is not retrained by climate, and the analysis does not cover all climate zones or building types.
...
We revised Section 4.3 and the objective-scope discussion to state the current objective scope and add these systems as future extensions. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now the current workflow covers feasible residential block morphologies, analytic EUIt/EG/H target assignment, surrogate comparison, projection, and selected physical and climate-sensitive cases. Storage dispatch, EV charging demand, district-level grid exchange, tariff response, and distributed-energy operation are not modeled in the current revision.

This handling keeps the revision honest: the manuscript acknowledges these energy-system interactions as important, but it does not retrofit them into objectives or evidence that were not part of the current study.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made two corresponding revisions. First, we revised Section 4.3 to state that storage, EV charging, distributed-energy operation, and district grid exchange are outside the current model. Second, we added these systems to future work as extensions that require new objectives and evidence.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
...
We revised Sections 2.5, 3.3, and 3.4, Tables 4--5, and Figs. 7--8 to separate best scalarized DDPG candidate, retained-output contracts, fixed-reference descriptor-space HV and IGD, equal-size comparison, feasible-block projection, and selected physical-case interpretation. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now shows that NSGA-II is strong in descriptor-space HV and IGD under the fixed benchmark reference, while DDPG outcomes are preference dependent and projection changes morphology-level interpretation. No single statement from these criteria defines a universal best physical scenario.
...
We revised Sections 1.2, 4.1, and 4.3 to expand the comparison with simulation-based optimization, surrogate-assisted optimization, NSGA-II Pareto archives, DRL control studies, and neighbouring derivative-free methods. The revised material now gives the reader a more direct account of what was changed and how the evidence should be read.

The evidence now states the advantage narrowly as transparent separation of fast descriptor-space screening, feasible-block projection, and selected sensitivity analysis. The current evidence supports representation-aware early-stage design support, not comprehensive superiority over existing approaches.

The response therefore compares mechanisms rather than making a broad advantage claim. The stated contribution is that the workflow exposes representation and evaluation boundaries that can be hidden in conventional optimizer comparisons. This keeps the comparison useful for reviewers who want to know what the framework adds while preserving the limits imposed by the current evidence. It also names the remaining evidence needed for any stronger comparison.
}
\responseheading{Revisions made in the manuscript.}
\responsepara{%
We made three corresponding revisions. First, we expanded the literature review to position the workflow relative to existing optimization and DRL studies. Second, we revised Section 4.1 to explain the representation-aware advantage. Third, we revised Section 4.3 to keep the comparison bounded by the current evidence.
}
\responseheading{Relevant revised manuscript and supporting evidence.}
\begin{revisionbox}
