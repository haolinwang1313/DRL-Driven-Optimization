...
\author{Haolin Wang, Zhi Wu, Wei Gu, Xiao Zang, Pengxiang Liu, Qirun Sun, and Wei Wang}
\date{}
\maketitle

This Supplementary Information provides case settings, analytic response-generation details, surrogate robustness results, optimizer comparison details, feasible-projection results, physics-based cross-model stress-test details, and cross-climate case results. The supplementary results are intended to make the main manuscript reproducible and interpretable. They do not extend the main manuscript into a claim of optimizer superiority under physical simulation.
...
\begin{tabularx}{\linewidth}{@{}p{0.12\linewidth}p{0.18\linewidth}rrrrr@{}}
\toprule
Prototype & Category & Footprint area & Min floors & Max floors & Width & Depth \\
\midrule
...
\footnotesize
\item Areas are in m$^2$, and linear dimensions are in m.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The active benchmark dataset is generated through the fallback analytic response generator. The generator maps morphology descriptors to EUIt, EG, and H through explicit formulas that include orientation penalty, aspect ratio, openness, sky-view factor, density, and a morphology-dependent sweet-spot term. The output values are clipped to the target ranges used during surrogate training. This design creates a controlled response surface for optimizer comparison and should not be described as a direct EnergyPlus/Radiance dataset.
...
For Dongtai station weather, the fallback station-bias terms are zero. The unclipped analytic responses are
...
\caption{Descriptor distributions, OSLI frequency, and nearest-neighbour distance distribution for the canonical 2000-row generated morphology pool.}
...
\caption{Dataset-scale surrogate-selection comparison across 500, 1000, 1500, and 2000 rows. The panels summarize target error, tail error, mean $R^2$, and the selection objective for the regime winners.}
\label{fig:s3-scale-study}
\end{figure}
...
\caption{Residual assessment for EUIt, EG, and H under repeated cross-validated surrogate predictions. The figure summarizes analytic-target prediction error and does not represent physical simulation error.}
\label{fig:s2-residual-assessment}
\end{figure}
...
Each DDPG scenario uses 20 seeds, 600 episodes, and 40 surrogate interactions per episode. The seed-level summaries record best reward, final reward, plateau episode, and best-to-final regression. These quantities describe policy-training behaviour under serialized surrogate queries and should not be interpreted as physical block evolution.

\setcounter{figure}{3}
\begin{figure}[H]
\centering
\sifigure{B1_seed_\diagword.pdf}
\caption{Seed-level DDPG training assessment across the three preference scenarios. The panels summarize best reward, final reward, plateau episode, and late-regression behaviour.}
\label{fig:s4-seed-assessment}
\end{figure}

\subsection*{S3.2 Optimizer output contracts}

DDPG retains one best scalarized candidate per seed, while NSGA-II retains final populations. CMA-ES and RandomSearch retain top-reward queried archives for secondary comparison, and FeasiblePoolRandom provides a deterministic feasible-pool reference. The retained-output contract prevents full-archive sizes from being interpreted as equivalent across optimizers.

\subsection*{S3.3 Equal-size sensitivity}

The main manuscript reports equal-size results at retained size 20 because that is the valid common retained size for DDPG and NSGA-II. Oversized equal-size requests for smaller source archives are labelled not applicable rather than reported as valid metric estimates.

\begin{table}[H]
\caption{Equal-size-20 descriptor-space benchmark metrics under benchmark-reference-v2.}
...
\caption{Median morphology descriptor signatures for representative retained-output groups. The figure describes descriptor-space tendencies rather than physical design rules.}
\label{fig:s5-morphology-signatures}
\end{figure}
...
\caption{Selected surrogate response profiles for OSR to EUIt, FAR to EG, SVF to H, and theta to H. The profiles illustrate local response-surface behaviour within the selected surrogate.}
\label{fig:s8-response-profiles}
\end{figure}
...
\caption{Optimizer-linked six-case projection and cross-model gap decomposition for EUIt, EG, and H. These cases bridge descriptor-space candidates, nearest generated feasible blocks, and physical outputs.}
\label{fig:s7-optimizer-linked-gaps}
\end{figure}

\subsection*{S5.3 Per-case physical results}

The 18 direct feasible cases completed both EnergyPlus and Radiance-component runs. EUIt agreement is weak, EG is represented by a GHI-based rooftop-PV proxy, and H is represented by January 20 windowsill direct-sun hours. The reported nMAE and rank-correlation values therefore define a stress-test boundary rather than a general physical ranking result.
...
\midrule
EUIt & 18 & 3.788 & 0.340 & -0.404 & weak metric agreement \\
...
The round-2 physical package records EG roof irradiance as unavailable for the direct stress-test metric table. The main manuscript therefore reports the GHI-based rooftop-PV proxy for EG and labels it explicitly. This avoids treating the proxy as a full rooftop-irradiance result.

\section*{S6. Cross-climate case details}

\subsection*{S6.1 Weather metadata}

The cross-climate sensitivity analysis uses Dongtai as the baseline station and evaluates the same four generated feasible blocks under Harbin, Beijing, and Guangzhou weather. The climate buckets are severe cold for Harbin, cold for Beijing, and hot-summer warm-winter for Guangzhou. The TMY period is 2009--2023 in the round-2 configuration.

\subsection*{S6.2 Four-block climate results}

The climate sensitivity set uses the same four block morphologies under three additional EPW files. The surrogate is not retrained, and the morphology set is not reselected after observing the climate results. The results therefore describe weather sensitivity for selected block morphologies rather than surrogate transfer across climate regions.

\begin{table}[H]
\caption{Station-level mean physical changes relative to Dongtai.}
\label{tab:s-climate-summary}
\centering
\footnotesize
\begin{tabularx}{\linewidth}{@{}p{0.22\linewidth}rrrp{0.24\linewidth}@{}}
\toprule
Station & $\Delta$EUIt & $\Delta$EG & $\Delta$H & Interpretation \\
\midrule
Beijing & 13.467 & 0.135 & -0.056 & cold-climate increase in EUIt \\
Guangzhou & -2.972 & -0.057 & -0.025 & warm-climate EUIt decrease \\
Harbin & 55.681 & -0.044 & -0.675 & severe-cold EUIt increase \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{Case-level climate sensitivity for EUIt, EG, and H across Beijing, Guangzhou, and Harbin. The heatmaps report physical changes relative to Dongtai for the four selected blocks.}
\label{fig:s9-climate-detail}
\end{figure}

\subsection*{S6.3 Rank stability across climates}

EG rank stability is high across the tested climates, with Spearman rho = 1.0 for Beijing, Guangzhou, and Harbin. H rank stability is low, with Spearman rho = 0.2 for Beijing, 0.4 for Guangzhou, and 0.2 for Harbin. EUIt rank stability is high for Beijing and Guangzhou but low for Harbin, with Spearman rho = 0.2. These results support target-specific climate-sensitivity reporting rather than broad cross-climate surrogate generalization.
